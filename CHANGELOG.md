...
- **breaking** removed the `cast` (django) command and turned it into
  ...
  of `write`
- engine: map specs gain user-defined template transforms, written in [starlark](https://github.com/bazelbuild/starlark) (#99). a `transforms:` block (`file:` or `inline:`) makes every top-level `def` available to `${var|...}` pipelines, consulted before the built-in four (so a user transform may shadow them). transforms take literal arguments (`${x|f|g(2)}` is `g(f(x), 2)`: quoted strings, ints, floats, bools), and a transformed value keeps its starlark type in `attrs:` templates (str/int/bool/list/dict mapped to json, so structured derivations land in `json`-typed attrs); `key:` templates and other string contexts coerce scalar results and reject collections, and untransformed templates behave exactly as before. `fail()` surfaces in the usual `rule <name>: ... in <context>` error shape. transforms are hermetic (no i/o, no `while`, no recursion) and `load()` is restricted to relative paths resolved against the spec file, transitively. behind a new `starlark` cargo feature on `alembic-engine` (off by default for library consumers; the cli ships with it enabled), pulling the `starlark` crate and its tree (cargo-deny: BSL-1.0/Zlib license allowances and four unmaintained-only advisory ignores, all transitive). the test suite reimplements the built-in four in starlark and asserts equivalence, as a standing proof the substrate is sufficient
- cli: add `map transform` subcommand (`alembic map transform --spec <map> <name> <json-value> [json-args...]`) evaluating a single transform against a single value, the iteration loop for writing transforms: the typed result prints as json, `fail()` exits non-zero with the message (#99). the flat `alembic map -f/--spec/-o` form is unchanged
