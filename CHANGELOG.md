...
- `ExternalAdapter` in `alembic-adapter-sdk` declares `type Error: Display`, and its methods return `Result<_, Self::Error>`; `type Error = anyhow::Error;` keeps an existing adapter's bodies unchanged
