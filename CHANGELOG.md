...
- core: the enum-schema-missing-`values` error now reads `enum type requires values` from both schema-parse paths (the nested type-object form had drifted to `enum type requires values array`), and the five compound field types are parsed by one shared helper so the two paths can no longer diverge
