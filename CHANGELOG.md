...
- engine: make the default (local file) state backend's write path crash-safe and durable (#44, #46). `LocalBackend::save` now performs the canonical durable atomic write — fsync the temp file's data before the rename, then fsync the parent directory so the rename itself survives a crash — instead of `write`+`rename` with no flush, which on a crash/power-loss could leave the state file truncated or empty and orphan backend objects. the temp file is now per-process (`<path>.<pid>.tmp`) to avoid colliding with a crashed or concurrent run and is best-effort cleaned up on failure, and a present-but-corrupt state file now loads with an actionable error pointing at the path. happy-path on-disk output is byte-identical
