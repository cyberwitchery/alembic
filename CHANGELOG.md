...
- **breaking** `apply` rejects a plan file carrying an unknown key rather than dropping it, `schema_preview` included
- **breaking** external adapters: an unknown key in a provisioning report, an applied operation or the response envelope is now rejected
