...
- import observes in the canonical uid space: it ignores the state store, so refs come back as canonical uids rather than the state-mapped ones `plan` observes. a ref the adapter can only report as a backend id is resolved against a `backend id -> canonical uid` index built from the observation itself, to a fixpoint since a key field can itself be a ref. a ref in a cycle, or to an object outside the observation, stays a backend id and validation names it
