...
alembic is a data-model-first converger + loader for dcim/ipam systems. it defines a canonical, vendor-neutral ir (intermediate representation) and an engine that validates, plans, and applies changes to a backend. it supports multiple backend adapters, currently targeting NetBox and Nautobot.
...
- **state store**: `.alembic/state.json` maps `uid` to backend ids (integers or uuids) per type to keep renames stable.
